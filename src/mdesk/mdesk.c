...
{
  String8List strs = {0};
  if (flags & MD_TokenFlag_Identifier)
  {
    str8_list_push(arena, &strs, str8_lit("Identifier"));
  }
  if (flags & MD_TokenFlag_Numeric)
  {
    str8_list_push(arena, &strs, str8_lit("Numeric"));
  }
  if (flags & MD_TokenFlag_StringLiteral)
  {
    str8_list_push(arena, &strs, str8_lit("StringLiteral"));
  }
  if (flags & MD_TokenFlag_Symbol)
  {
    str8_list_push(arena, &strs, str8_lit("Symbol"));
  }
  if (flags & MD_TokenFlag_Reserved)
  {
    str8_list_push(arena, &strs, str8_lit("Reserved"));
  }
  if (flags & MD_TokenFlag_Comment)
  {
    str8_list_push(arena, &strs, str8_lit("Comment"));
  }
  if (flags & MD_TokenFlag_Whitespace)
  {
    str8_list_push(arena, &strs, str8_lit("Whitespace"));
  }
  if (flags & MD_TokenFlag_Newline)
  {
    str8_list_push(arena, &strs, str8_lit("Newline"));
  }
  if (flags & MD_TokenFlag_BrokenComment)
  {
    str8_list_push(arena, &strs, str8_lit("BrokenComment"));
  }
  if (flags & MD_TokenFlag_BrokenStringLiteral)
  {
    str8_list_push(arena, &strs, str8_lit("BrokenStringLiteral"));
  }
  if (flags & MD_TokenFlag_BadCharacter)
  {
    str8_list_push(arena, &strs, str8_lit("BadCharacter"));
  }
  return strs;
}
...
md_node_rec_depth_first(MD_Node *node, MD_Node *subtree_root, U64 child_off, U64 sib_off)
{
  ...
  if (!md_node_is_nil(*MemberFromOffset(MD_Node **, node, child_off)))
  {
    ...
    for (MD_Node *p = node; !md_node_is_nil(p) && p != subtree_root; p = p->parent, rec.pop_count += 1)
    {
  ...
  MD_Node *dst_parent = dst_root;
  {
    ...
    for (MD_Node *src = src_root; !md_node_is_nil(src); src = rec.next)
    {
      ...
      if (rec.push_count != 0)
      {
        ...
        for (U64 idx = 0; idx < rec.pop_count; idx += 1)
        {
...
#define MD_ParseWorkPush(work_kind, work_parent)                  \
  do                                                              \
  {                                                               \
    ...
    for (MD_Node *node = root, *next = &md_nil_node; !md_node_is_nil(node); node = next)
    {
      ...
      switch (node->kind)
      {
        ...
        case MD_NodeKind_File: {
          kind_string = str8_lit("File");
        }
        break;
        case MD_NodeKind_ErrorMarker: {
          kind_string = str8_lit("ErrorMarker");
        }
        break;
        case MD_NodeKind_Main: {
          kind_string = str8_lit("Main");
        }
        break;
        case MD_NodeKind_Tag: {
          kind_string = str8_lit("Tag");
        }
        break;
        case MD_NodeKind_List: {
          kind_string = str8_lit("List");
        }
        break;
        case MD_NodeKind_Reference: {
          kind_string = str8_lit("Reference");
        }
        break;
      }
    ...
    for (MD_Node *node = root, *next = &md_nil_node; !md_node_is_nil(node); node = next)
    {
      ...
      // rjf: popping -> close braces
      {
        ...
        for (S32 pop_idx = 0; pop_idx < rec.pop_count; pop_idx += 1, popped = popped->parent)
        {
          ...
          if (popped->flags & MD_NodeFlag_HasParenRight)
          {
            closer_symbol = s(")");
          }
          if (popped->flags & MD_NodeFlag_HasBracketRight)
          {
            closer_symbol = s("]");
          }
          if (popped->flags & MD_NodeFlag_HasBraceRight)
          {
            closer_symbol = s("}");
          }
