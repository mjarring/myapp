      ...
      if (str8_match(file_ext, str8_lit("mdesk"), 0))
      {
        ...
        for (MD_Msg *m = parse.msgs.first; m != 0; m = m->next)
        {
          ...
          switch (m->kind)
          {
            ...
            break;
            case MD_MsgKind_Note: {
              msg_kind_string = str8_lit("note");
            }
            break;
            case MD_MsgKind_Warning: {
              msg_kind_string = str8_lit("warning");
            }
            break;
            case MD_MsgKind_Error: {
              msg_kind_string = str8_lit("error");
            }
            break;
            case MD_MsgKind_FatalError: {
              msg_kind_string = str8_lit("fatal error");
            }
            break;
          }
    ...
    for
      MD_EachNode(node, file->first)
      {
        ...
        if (!md_node_is_nil(tag))
        {
          ...
          else if (str8_match(tag->first->string, str8_lit("enums"), 0))
          {
            out = &layer->enums;
          }
          else if (str8_match(tag->first->string, str8_lit("structs"), 0))
          {
            out = &layer->structs;
          }
          else if (str8_match(tag->first->string, str8_lit("functions"), 0))
          {
            out = prefer_c_file ? &layer->c_functions : &layer->h_functions;
          }
          else if (str8_match(tag->first->string, str8_lit("tables"), 0))
          {
        ...
        if (make_directory(layer_generated_folder))
        {
          ...
          }
          {
            ...
            if (layer->h_header.first == 0)
            {
              ...
              for (String8Node *n = layer->h_header.first; n != 0; n = n->next)
              {
            ...
            if (layer->h_footer.first == 0)
            {
              ...
              for (String8Node *n = layer->h_footer.first; n != 0; n = n->next)
              {
                fwrite(n->string.str, n->string.size, 1, h);
              }
            fclose(h);
          }
          {
            ...
            if (layer->c_header.first == 0)
            {
              ...
              for (String8Node *n = layer->c_header.first; n != 0; n = n->next)
              {
