...
static inline void sha256_update(sha256_ctx *ctx, const void *data,
                                 size_t size);
static inline void sha256_finish(sha256_ctx *ctx,
                                 uint8_t digest[SHA256_DIGEST_SIZE]);

static inline void sha224_init(sha224_ctx *ctx);
static inline void sha224_update(sha224_ctx *ctx, const void *data,
                                 size_t size);
static inline void sha224_finish(sha224_ctx *ctx,
...
#define SHA256_GET32BE(ptr)                                                    \
  _byteswap_ulong(*((const __unaligned uint32_t *)(ptr)))
#define SHA256_SET32BE(ptr, x)                                                 \
  *((__unaligned uint32_t *)(ptr)) = _byteswap_ulong(x)
#define SHA256_SET64BE(ptr, x)                                                 \
  *((__unaligned uint64_t *)(ptr)) = _byteswap_uint64(x)
#else
#define SHA256_GET32BE(ptr)                                                    \
...
#define SHA256_CPUID_EX(x, y, info)                                            \
...
static void sha256_process_shani(uint32_t *state, const uint8_t *block,
                                 size_t count) {
...
// there is tricky dependency for lanes 2 and 3 on result of lanes 0 and 1, but
// sha256msg2 op takes care of that
...
// rounds i>2: m1 = _mm_sha256msg2_epu32(_mm_add_epi32(m1, _mm_alignr_epi8(m0,
// m3, 4) ), m0) rounds i>0: m3 = _mm_sha256msg1_epu32(m3, m0)

// round functions are done with _mm_sha256rnds2_epu32 which performs it for 2
// rounds thus repeat it two times, as input use m0 + K(i) - message schedule
// added with sha256 constants

// 4 wide round calculations
#define QROUND(i, m0, m1, m2, m3)                                              \
  do {                                                                         \
    /* update message schedule */                                              \
    if (i > 2 && i < 15)                                                       \
      m1 = _mm_sha256msg2_epu32(_mm_add_epi32(m1, _mm_alignr_epi8(m0, m3, 4)), \
                                m0);                                           \
    if (i > 0 && i < 13)                                                       \
      m3 = _mm_sha256msg1_epu32(m3, m0);                                       \
    /* add round constants */                                                  \
    __m128i tmp =                                                              \
        _mm_add_epi32(m0, _mm_loadu_si128((const __m128i *)&SHA256_K[4 * i])); \
    /* 4 round functions */                                                    \
    s1 = _mm_sha256rnds2_epu32(s1, s0, tmp);                                   \
    s0 = _mm_sha256rnds2_epu32(                                                \
        s0, s1, _mm_shuffle_epi32(tmp, _MM_SHUFFLE(0, 0, 3, 2)));              \
  } while (0)
  ...
  // load initial state
  __m128i abcd = _mm_shuffle_epi32(_mm_loadu_si128((const __m128i *)&state[0]),
                                   _MM_SHUFFLE(0, 1, 2, 3)); // [a,b,c,d]
  __m128i efgh = _mm_shuffle_epi32(_mm_loadu_si128((const __m128i *)&state[4]),
                                   _MM_SHUFFLE(0, 1, 2, 3)); // [e,f,g,h]
  ...
  _mm_storeu_si128((__m128i *)&state[0],
                   _mm_shuffle_epi32(abcd, _MM_SHUFFLE(0, 1, 2, 3)));
  _mm_storeu_si128((__m128i *)&state[4],
                   _mm_shuffle_epi32(efgh, _MM_SHUFFLE(0, 1, 2, 3)));
...
static inline int sha256_cpuid(void) {
  ...
  if (result == 0) {
    ...
    int has_arm64 = sysctlbyname("hw.optional.arm.FEAT_SHA256", &value,
                                 &valuelen, NULL, 0) == 0 &&
                    value != 0;
#else
...
SHA256_TARGET
static void sha256_process_arm64(uint32_t *state, const uint8_t *block,
                                 size_t count) {
#define QROUND(i, m0, m1, m2, m3)                                              \
  do {                                                                         \
    /* update message schedule */                                              \
...
static void sha256_process(uint32_t *state, const uint8_t *block,
                           size_t count) {
#if defined(__x86_64__) || defined(_M_AMD64)
...
#define BSig0(x)                                                               \
  (SHA256_ROR32(x, 2) ^ SHA256_ROR32(x, 13) ^ SHA256_ROR32(x, 22))
#define BSig1(x)                                                               \
  (SHA256_ROR32(x, 6) ^ SHA256_ROR32(x, 11) ^ SHA256_ROR32(x, 25))
...
#define ROUND(i, a, b, c, d, e, f, g, h)                                       \
  do {                                                                         \
    uint32_t w0;                                                               \
    if (i < 16)                                                                \
      W(i) = w0 = SHA256_GET32BE(block + i * sizeof(uint32_t));                \
    if (i >= 16)                                                               \
      W(i) = w0 = SSig1(W(i - 2)) + W(i - 7) + SSig0(W(i - 15)) + W(i - 16);   \
                                                                               \
...
void sha256_finish(sha256_ctx *ctx, uint8_t digest[SHA256_DIGEST_SIZE]) {
  ...
  SHA256_SET64BE(padding + blocks * SHA256_BLOCK_SIZE - sizeof(bitcount),
                 bitcount);
