...
#define MD5_GET32LE(ptr)                                                       \
  (((ptr)[0] << 0) | ((ptr)[1] << 8) | ((ptr)[2] << 16) | ((ptr)[3] << 24))
...
// MD5_COMPILER_BARRIER forces clang to do better codegen without spilling
// registers to stack too much
...
static void md5_process_bmi2(uint32_t *state, const uint8_t *block,
                             size_t count) {
  // "tune=znver1" allows clang to use LEA with [reg+reg+imm] operand which
  // helps performance on modern CPU's -1 in I will get folded together with
  // constant k
...
// F function uses 3 operations instead of 4 when "bit select" instruction is
// not available (x & y) | (~x & z) == (z ^ (x & (y ^ z))
