...
static inline void sha512_update(sha512_ctx *ctx, const void *data,
                                 size_t size);
static inline void sha512_finish(sha512_ctx *ctx,
                                 uint8_t digest[SHA512_DIGEST_SIZE]);

static inline void sha384_init(sha384_ctx *ctx);
static inline void sha384_update(sha384_ctx *ctx, const void *data,
                                 size_t size);
static inline void sha384_finish(sha384_ctx *ctx,
...
#define SHA512_GET64BE(ptr)                                                    \
  _byteswap_uint64(*((const __unaligned uint64_t *)(ptr)))
#define SHA512_SET64BE(ptr, x)                                                 \
...
#define SHA512_CPUID_EX(x, y, info)                                            \
...
static void sha512_process_vsha512(uint64_t *state, const uint8_t *block,
                                   size_t count) {
// pretty much same way how sha256 works, only with avx2 registers and 64-bit
// additions state is kept as two 256-bit ymm registers (8 qwords)
...
#define QROUND(i, m0, m1, m2, m3)                                              \
  do {                                                                         \
    ...
    if (i > 2 && i < 19)                                                       \
      m1 = _mm256_sha512msg2_epi64(                                            \
          _mm256_add_epi64(                                                    \
              m1, _mm256_permute4x64_epi64(_mm256_blend_epi32(m3, m0, 3),      \
                                           _MM_SHUFFLE(0, 3, 2, 1))),          \
          m0);                                                                 \
    if (i > 0 && i < 17)                                                       \
      m3 = _mm256_sha512msg1_epi64(m3, _mm256_castsi256_si128(m0));            \
    /* add round constants */                                                  \
    __m256i tmp = _mm256_add_epi64(                                            \
        m0, _mm256_loadu_si256((const __m256i *)&SHA512_K[4 * i]));            \
    /* 4 round functions */                                                    \
  ...
  // load initial state
  __m256i abcd =
      _mm256_permute4x64_epi64(_mm256_loadu_si256((const __m256i *)&state[0]),
                               _MM_SHUFFLE(0, 1, 2, 3)); // [a,b,c,d]
  __m256i efgh =
      _mm256_permute4x64_epi64(_mm256_loadu_si256((const __m256i *)&state[4]),
  ...
  // save the new state
  _mm256_storeu_si256((__m256i *)&state[0],
                      _mm256_permute4x64_epi64(abcd, _MM_SHUFFLE(0, 1, 2, 3)));
  _mm256_storeu_si256((__m256i *)&state[4],
                      _mm256_permute4x64_epi64(efgh, _MM_SHUFFLE(0, 1, 2, 3)));
...
static inline int sha512_cpuid(void) {
  ...
  if (result == 0) {
    ...
    RegGetValueA(HKEY_LOCAL_MACHINE,
                 ...
                 "CP 4030", RRF_RT_QWORD | RRF_ZEROONFAILURE, NULL, &bits,
                 &bitsize);
    ...
    int has_arm64 = sysctlbyname("hw.optional.arm.FEAT_SHA512", &value,
                                 &valuelen, NULL, 0) == 0 &&
                    value != 0;
#else
...
SHA512_TARGET
static void sha512_process_arm64(uint64_t *state, const uint8_t *block,
                                 size_t count) {
#define DROUND(i, ab, cd, ef, gh, m0, m1, m2, m3, m4, m5, m6, m7)              \
  do {                                                                         \
    /* update message schedule */                                              \
    if (i >= 8)                                                                \
...
static void sha512_process(uint64_t *state, const uint8_t *block,
                           size_t count) {
#if defined(__x86_64__) || defined(_M_AMD64)
...
#define BSig0(x)                                                               \
  (SHA512_ROR64(x, 28) ^ SHA512_ROR64(x, 34) ^ SHA512_ROR64(x, 39))
#define BSig1(x)                                                               \
  (SHA512_ROR64(x, 14) ^ SHA512_ROR64(x, 18) ^ SHA512_ROR64(x, 41))
...
#define ROUND(i, a, b, c, d, e, f, g, h)                                       \
  do {                                                                         \
    uint64_t w0;                                                               \
    if (i < 16)                                                                \
      W(i) = w0 = SHA512_GET64BE(block + i * sizeof(uint64_t));                \
    if (i >= 16)                                                               \
...
void sha512_finish(sha512_ctx *ctx, uint8_t digest[SHA512_DIGEST_SIZE]) {
  ...
  SHA512_SET64BE(padding + blocks * SHA512_BLOCK_SIZE - 2 * sizeof(uint64_t),
                 bitcount[1]);
  SHA512_SET64BE(padding + blocks * SHA512_BLOCK_SIZE - 1 * sizeof(uint64_t),
                 bitcount[0]);
