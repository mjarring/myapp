...
#define access_pt_is_expired(pt, ...)                                          \
  access_pt_is_expired_(                                                       \
      (pt),                                                                    \
      &(AccessPtExpireParams){.time = 2000000, .update_idxs = 2, __VA_ARGS__})

//- rjf: progress counters
#define set_progress_ptr(ptr) (tctx_selected()->progress_counter_ptr = (ptr))
#define set_progress_target_ptr(ptr)                                           \
  (tctx_selected()->progress_target_ptr = (ptr))
#define set_progress(val)                                                      \
  (tctx_selected()->progress_counter_ptr                                       \
       ? ins_atomic_u64_eval_assign(tctx_selected()->progress_counter_ptr,     \
                                    (val))                                     \
       : (void)0)
#define add_progress(val)                                                      \
  (tctx_selected()->progress_counter_ptr                                       \
       ? ins_atomic_u64_add_eval(tctx_selected()->progress_counter_ptr, (val)) \
       : (void)0)
#define set_progress_target(val)                                               \
  (tctx_selected()->progress_target_ptr                                        \
       ? ins_atomic_u64_eval_assign(tctx_selected()->progress_target_ptr,      \
                                    (val))                                     \
       : (void)0)
