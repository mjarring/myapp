...
internal Arena *arena_alloc_(ArenaParams *params)
{
  ...
  if (base == 0)
  {
    ...
    {
      reserve_size =
          AlignPow2(reserve_size, get_system_info()->large_page_size);
  ...
  if (Unlikely(base == 0))
  {
    wm_graphical_message(1, str8_lit("Fatal Allocation Failure"),
  ...
  {
    B32 got_lock =
        (ins_atomic_u64_eval_cond_assign(&arena_table_lock, 1, 0) == 0);
    if (got_lock)
    {
      ...
      else
      {
        ...
        if (arena_table_count >= arena_table_cap)
        {
          ...
          commit_memory(arena_table + arena_table_cap__pre_grow,
                        sizeof(arena_table[0]) * (arena_table_cap__post_grow -
                                                  arena_table_cap__pre_grow));
        }
        arena_table_count += 1;
      }
      node->arena = arena;
...
internal void arena_release(Arena *arena)
{
#if PROFILE_TELEMETRY
  {
    ...
    while (base_arena->prev)
      base_arena = base_arena->prev;
    tmPlot(0, TM_PLOT_UNITS_MEMORY, TM_PLOT_DRAW_LINE, 0, "%s/%p",
           base_arena->name, base_arena);
  }
#endif
...
internal void *arena_push(Arena *arena, U64 size, U64 align, B32 zero)
{
  ...
  if (size > KB(1))
  {
    ...
    while (base_arena->prev)
      base_arena = base_arena->prev;
    tmPlot(0, TM_PLOT_UNITS_MEMORY, TM_PLOT_DRAW_LINE,
           (double)(current->base_pos + current->pos) / 1024.0 / 1024.0,
           "%s/%p", base_arena->name, base_arena);
  }
#endif
