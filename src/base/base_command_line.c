...
internal CmdLine cmd_line_from_string_list(Arena *arena,
  ...
  for (String8Node *node = command_line.first->next, *next = 0; node != 0;
    ...
    if (is_option) {
      ...
      B32 has_values = 0;
      U64 value_signifier_position1 =
          str8_find_needle(option_name, 0, str8_lit(":"), 0);
      U64 value_signifier_position2 =
          str8_find_needle(option_name, 0, str8_lit("="), 0);
      U64 value_signifier_position =
          Min(value_signifier_position1, value_signifier_position2);
      String8 value_portion_this_string =
          str8_skip(option_name, value_signifier_position + 1);
...
internal CmdLineOpt *cmd_line_opt_from_string(CmdLine *cmd_line, String8 name) {
  return cmd_line_opt_from_slot(cmd_line_slot_from_string(cmd_line, name),
                                name);
}
