    ...
    for (SerializeTypeTask *t = first_task; t != 0; t = t->next)
    {
      switch (t->type->kind)
      {
        ...
        case TypeKind_Ptr: {
          ...
          // rjf: indexification -> subtract base, divide direct size, write
          // index
          if (ptr_ref_info != 0 && ptr_ref_info->indexify_base != 0)
          {
            ...
            U64 ptr_write_value =
                ((U64)((U8 *)ptr_value - (U8 *)ptr_ref_info->indexify_base) /
                 t->type->direct->size);
          ...
          else if (ptr_ref_info != 0 && ptr_ref_info->offsetify_base != 0)
          {
            ...
            MemoryCopy(&ptr_value, t->src, sizeof(ptr_value));
            U64 ptr_write_value =
                (U64)((U8 *)ptr_value - (U8 *)ptr_ref_info->offsetify_base);
            str8_serial_push_struct(scratch.arena, &strings, &ptr_write_value);
          }

          // rjf: size-by-member (pre-header): still potentially dependent on
          // other members which delimit our size, so push a new post-header
          // task for pointer.
          ...
          // struct have been iterated, so now we can read the size, & descend
          // to new task to read pointer destination contents
          else if (t->type->count_delimiter_name.size != 0 && t->is_post_header)
          {
            // rjf: determine count of this pointer
            U64 count = 0;
            {
              Member *count_member = member_from_name(
                  t->containing_type, t->type->count_delimiter_name);
              MemoryCopy(&count, t->containing_ptr + count_member->value,
                         count_member->type->size);
            }
        ...
        case TypeKind_Struct: {
          ...
          for (U64 idx = 0; idx < t->count; idx += 1)
          {
                 ...
                 member_idx += 1)
            {
              ...
              task->count           = 1;
              task->src             = t->src + idx * t->type->size +
                                      t->type->members[member_idx].value;
      ...
      switch (t->type->kind)
      {
        ...
        case TypeKind_Ptr: {
          ...
          if (ptr_ref_info != 0 && ptr_ref_info->indexify_base != 0)
          {
            ...
            U64 ptr_write_value =
                (ptr_value + (U64)ptr_ref_info->indexify_base) *
                t->type->direct->size;
          ...
          else if (t->type->count_delimiter_name.size != 0 && t->is_post_header)
          {
            ...
            // rjf: allocate buffer for pointer destination; write address into
            // pointer value slot
        ...
        case TypeKind_Struct: {
          for (U64 idx = 0; idx < t->count; idx += 1)
          {
                 ...
                 member_idx += 1)
            {
              ...
              task->type            = t->type->members[member_idx].type;
              task->count           = 1;
              task->dst             = t->dst + idx * t->type->size +
                                      t->type->members[member_idx].value;
              task->containing_type = t->type;
                                           ...
                                           TypeSerializeParams *params)
{
  ...
  String8 data_copy =
      deserialized_from_typed_data(arena, type, data_srlz, params);
  scratch_end(scratch);
  return data_copy;
}
