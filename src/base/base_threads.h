...
#define RWMutexScope(mutex, write_mode)                                        \
  DeferLoop(rw_mutex_take((mutex), (write_mode)),                              \
            rw_mutex_drop((mutex), (write_mode)))
#define MutexScopeR(mutex)                                                     \
  DeferLoop(rw_mutex_take_r(mutex), rw_mutex_drop_r(mutex))
#define MutexScopeW(mutex)                                                     \
  DeferLoop(rw_mutex_take_w(mutex), rw_mutex_drop_w(mutex))
#define MutexScopeRWPromote(mutex)                                             \
  DeferLoop((rw_mutex_drop_r(mutex), rw_mutex_take_w(mutex)),                  \
            (rw_mutex_drop_w(mutex), rw_mutex_take_r(mutex)))
