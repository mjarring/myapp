...
#define ProfBeginDynamic(...)                                                  \
  (TM_API_PTR ? TM_API_PTR->_tmEnterZoneV_Core(0, 0, __FILE__,                 \
...
#define ProfBeginCapture(...)                                                  \
  tmOpen(0, __VA_ARGS__, __DATE__, "localhost", TMCT_TCP,                      \
...
#define ProfThreadName(...)                                                    \
  (TM_API_PTR ? TM_API_PTR->_tmThreadName(0, 0, __VA_ARGS__) : (void)0)
#define ProfMsg(...)                                                           \
  (TM_API_PTR ? TM_API_PTR->_tmMessageV_Core(0, TMMF_ICON_NOTE, __FILE__,      \
                                             &g_telemetry_filename_id,         \
                                             __LINE__, __VA_ARGS__)            \
              : (void)0)
...
#define ProfEnd(...)                                                           \
  (spall_capturing                                                             \
   ? (spall_buffer_end_ex(&spall_profile, &spall_buffer, now_time_us(),        \
                          spall_tid, spall_pid)),                              \
   0 : 0)
#define ProfTick(...)
