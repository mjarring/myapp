...
internal DenseTime lnx_dense_time_from_timespec(timespec in) {
  DenseTime result = 0;
  {
    ...
    DateTime date_time =
        lnx_date_time_from_tm(tm_time, in.tv_nsec / Million(1));
...
internal void sleep_ms(U32 ms) { usleep(ms * Thousand(1)); }
...
internal SystemInfo *get_system_info(void) { return &lnx_state.system_info; }
...
internal B32 semaphore_take(Semaphore semaphore, U64 endt_us) {
  ...
  if (semaphore.u64[0] != 0) {
    ...
    err = LNX_RETRY_ON_EINTR(
        sem_clockwait((sem_t *)semaphore.u64[0], CLOCK_MONOTONIC, &t));
  }
  return err == 0;
}
...
internal void abort_self(U64 exit_code) { exit((int)exit_code); }
...
int main(int argc, char **argv) {
  // install signal handler for the crash call stacks
  {
    struct sigaction handler = {
        .sa_sigaction = lnx_signal_handler,
        .sa_flags = SA_SIGINFO,
    };
