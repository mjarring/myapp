  ...
  if (!file_match(file, file_zero())) {
    ...
    U64 file_off = 0;
    {
      for (String8Node *n = list.first; n != 0; n = n->next) {
        for (U64 n_off = 0; n_off < n->string.size;) {
          ...
          if (write_buffer_available_size == 0) {
            U64 file_write_size =
                file_write(file, r1u64(file_off, file_off + write_buffer_size),
                           write_buffer);
            ...
            U64 bytes_to_copy =
                Min(write_buffer_available_size, n->string.size - n_off);
            write_buffer_write_pos += wrapped_write(
                write_buffer, write_buffer_size, write_buffer_write_pos,
                n->string.str + n_off, bytes_to_copy);
            n_off += bytes_to_copy;
          }
        }
      }
      if (write_buffer_write_pos > write_buffer_read_pos) {
        U64 file_write_size =
            file_write(file,
                       r1u64(file_off, file_off + (write_buffer_write_pos -
                                                   write_buffer_read_pos)),
                       write_buffer);
...
internal B32 append_data_to_file_path(String8 path, String8 data) {
  ...
  if (data.size != 0) {
    ...
    if (!file_match(file, file_zero())) {
      ...
      U64 bytes_written =
          file_write(file, r1u64(pos, pos + data.size), data.str);
      good = (bytes_written == data.size);
      file_close(file);
    }
  }
...
internal String8 string_from_file_range(Arena *arena, File file,
                                        Rng1U64 range) {
