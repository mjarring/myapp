...
typedef enum TypeKind {
  ...
  // rjf: leaves
  TypeKind_Void,
  TypeKind_FirstLeaf = TypeKind_Void,
  TypeKind_U8,
  TypeKind_U16,
  TypeKind_U32,
  TypeKind_U64,
  ...
  TypeKind_F32,
  TypeKind_F64,
  TypeKind_LastLeaf = TypeKind_F64,
...
#define member_lit_comp(S, ti, m, ...)                                         \
  {str8_lit_comp(#m), {0}, (ti), OffsetOf(S, m), __VA_ARGS__}
#define struct_members(S) read_only global Member S##__members[] =
#define struct_type(S, ...)                                                    \
  read_only global Type S##__type = {TypeKind_Struct,                          \
                                     0,                                        \
                                     sizeof(S),                                \
                                     &type_nil,                                \
                                     str8_lit_comp(#S),                        \
                                     {0},                                      \
                                     ArrayCount(S##__members),                 \
                                     S##__members,                             \
                                     __VA_ARGS__}
#define named_struct_type(name, S, ...)                                        \
  read_only global Type name##__type = {TypeKind_Struct,                       \
                                        0,                                     \
                                        sizeof(S),                             \
                                        &type_nil,                             \
                                        str8_lit_comp(#name),                  \
                                        {0},                                   \
                                        ArrayCount(name##__members),           \
                                        name##__members,                       \
                                        __VA_ARGS__}
#define ptr_type(name, ti, ...)                                                \
  read_only global Type name = {TypeKind_Ptr, 0, sizeof(void *), (ti),         \
                                __VA_ARGS__}
...
read_only global Type *type_kind_type_table[] = {
    ...
    type(B32), type(B64),  type(F32), type(F64), &type_nil, &type_nil,
    &type_nil, &type_nil,  &type_nil,
};
...
Member String8Node__members[] = {
    {str8_lit_comp("next"),
     ...
     OffsetOf(String8Node, next)},
    {str8_lit_comp("string"),
     {0},
     type(String8),
     OffsetOf(String8Node, string)},
};
...
Member String8List__members[] = {
    {str8_lit_comp("first"),
     {0},
     &String8Node__ptr_type,
     OffsetOf(String8List, first)},
    {str8_lit_comp("last"),
     {0},
     &String8Node__ptr_type,
     OffsetOf(String8List, last),
     MemberFlag_DoNotSerialize},
    {str8_lit_comp("node_count"),
     {0},
     type(U64),
     OffsetOf(String8List, node_count)},
    {str8_lit_comp("total_size"),
     {0},
     type(U64),
     OffsetOf(String8List, total_size)},
};
...
#define EachMember(T, it)                                                      \
  (Member *it = (type(T))->members;                                            \
   it != 0 && it < (type(T))->members + (type(T))->count; it += 1)
...
#define serialized_from_struct(arena, T, ptr, ...)                             \
  serialized_from_typed_data(                                                  \
      (arena), type(T), str8_struct(ptr),                                      \
      &(TypeSerializeParams){.ptr_ref_infos = 0, __VA_ARGS__})
#define struct_from_serialized(arena, T, string, ...)                          \
  (T *)deserialized_from_typed_data(                                           \
      (arena), type(T), (string),                                              \
      &(TypeSerializeParams){.ptr_ref_infos = 0, __VA_ARGS__})                 \
      .str
#define deep_copy_from_struct(arena, T, ptr, ...)                              \
  (T *)deep_copy_from_typed_data(                                              \
      (arena), type(T), str8_struct(ptr),                                      \
      &(TypeSerializeParams){.ptr_ref_infos = 0, __VA_ARGS__})                 \
      .str
