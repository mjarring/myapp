...
internal DenseTime mac_dense_time_from_timespec(timespec in)
{
  DenseTime result = 0;
  {
    ...
    DateTime date_time =
        mac_date_time_from_tm(tm_time, in.tv_nsec / Million(1));
...
internal void *reserve_memory_large(U64 size)
{
  void *result = mmap(0, size, PROT_NONE,
                      MAP_PRIVATE | MAP_ANONYMOUS, VM_FLAGS_SUPERPAGE_SIZE_ANY, 0);
  if (result == MAP_FAILED)
  {
    result = 0;
  }
...
internal B32 cond_var_wait(CondVar cv, Mutex mutex, U64 endt_us)
{
  ...
  int wait_result = pthread_cond_timedwait_relative_np(
...
{
  // TODO: mja: Is this accurate for mac?
...
internal B32 file_iter_next(Arena *arena, FileIter *iter, FileInfo *info_out)
{
  ...
  for (; mac_iter->dir != 0;)
  {
    ...
    {
      Temp    scratch   = scratch_begin(&arena, 1);
      String8 full_path = push_str8f(scratch.arena, "%S/%s", mac_iter->path,
                                     mac_iter->dp->d_name);
    ...
    if (good)
    {
      filtered =
          ((st.st_mode == S_IFDIR && iter->flags & FileIterFlag_SkipFolders) ||
           ...
           (mac_iter->dp->d_name[0] == '.' && mac_iter->dp->d_name[1] == '.' &&
            mac_iter->dp->d_name[2] == 0));
    }
    ...
    {
      info_out->name =
          push_str8_copy(arena, str8_cstring(mac_iter->dp->d_name));
      if (stat_result != -1)
      {
...
internal Process process_launch(ProcessLaunchParams *params)
{
  ...
  if (file_actions_init_code == 0)
  {
    ...
    int stdout_code = posix_spawn_file_actions_adddup2(
        &file_actions, (int)params->stdout_file.u64[0], STDOUT_FILENO);
    int stderr_code = posix_spawn_file_actions_adddup2(
        &file_actions, (int)params->stderr_file.u64[0], STDERR_FILENO);
    int stdin_code = posix_spawn_file_actions_adddup2(
...
internal B32 process_join(Process process, U64 endt_us, U64 *exit_code_out)
{
  ...
  {
    int   status      = 0;
    pid_t wait_result = mac_RETRY_ON_EINTR(
...
internal void mac_signal_handler(int sig, siginfo_t *info, void *arg)
{
...
{
  // TODO: mja install signal handler for the crash call stacks
  {
    struct sigaction handler = {
        .sa_sigaction = mac_signal_handler,
        .sa_flags     = SA_SIGINFO,
    };
  ...
  //- rjf: set up OS layer
  {
    ...
    // cache default environment
    {
      ...
      {
        default_env[idx] =
            (char *)str8_copy(mac_state.arena, str8_cstring(__environ[idx]))
                .str;
      }
    ...
    //- rjf: grab dynamically allocated system info
    {
      ...
      if (got_final_result && size > 0)
      {
        ...
        info->machine_name.str  = push_array_no_zero(
            mac_state.arena, U8, info->machine_name.size + 1);
      ...
      // rjf: grab program/user data paths
      {
        ...
        else
        {
          info->user_program_config_data_path =
              str8f(mac_state.arena, "%s/.config", home);
        }
        if (xdg_cache_home != 0)
        {
        ...
        else
        {
          info->user_program_cache_data_path =
              str8f(mac_state.arena, "%s/.cache", home);
        }
        if (xdg_state_home != 0)
        {
        ...
        else
        {
          info->user_program_logs_data_path =
              str8f(mac_state.arena, "%s/.local/state", home);
        }
      }
